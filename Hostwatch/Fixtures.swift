...
enum Fixtures {
    ...
    static let sources = Sources(windowHours: 24, site: nil,
        ...
        totalRequests: 9_881, classifiedRequests: 9_881, internalRequests: 948, externalRequests: 8_933,
        internalRoutes: [
    ...
    static let requests: [RequestSample] = (0..<72).map { index in
        ...
        let fromUs = index % 7 == 0
        let callers = ["applydjinn", "kablay-il", "granttap", "eppy-web"]
        return .init(id: "req-\(index)", time: ISO8601DateFormatter().string(from: Date().addingTimeInterval(Double(-index * 28))), site: site.id, host: site.domains[0], method: index % 6 == 0 ? "POST" : "GET", path: paths[index % paths.count], status: statuses[index % statuses.count], bytes: Double(437 + index * 89), requestBytes: Double(212 + index), durationMs: Double(2 + (index * 73) % 1200), scheme: "https", protocolName: "HTTP/2.0", tlsProtocol: "TLSv1.3", tlsCipher: "TLS_AES_256_GCM_SHA384", upstreamAddr: "127.0.0.1:3000", upstreamStatus: String(statuses[index % statuses.count]), upstreamMs: Double(1 + index % 83), cacheStatus: index % 4 == 0 ? "HIT" : "MISS", clientIp: fromUs ? "172.18.0.\(10 + index % 40)" : (index % 3 == 0 ? "203.0.113.\(20 + index)" : "198.51.100.\(10 + index)"), internalRequest: fromUs, country: fromUs ? "Private" : (index % 3 == 0 ? "Israel" : "United States"), countryCode: fromUs ? "" : (index % 3 == 0 ? "IL" : "US"), region: fromUs ? nil : (index % 3 == 0 ? "Tel Aviv" : "Virginia"), city: fromUs ? nil : (index % 3 == 0 ? "Tel Aviv" : "Ashburn"), latitude: fromUs ? nil : (index % 3 == 0 ? 32.0853 : 39.0438), longitude: fromUs ? nil : (index % 3 == 0 ? 34.7818 : -77.4874), clientService: fromUs ? callers[index % callers.count] : nil, userAgent: fromUs ? "hostwatch-internal" : (index % 5 == 0 ? "Googlebot/2.1" : "Safari/605.1.15"), source: fromUs ? "Internal" : (index % 5 == 0 ? "Googlebot" : "Direct"), referrerPath: fromUs ? nil : (index % 5 == 0 ? "/search" : nil), bot: fromUs ? nil : (index % 5 == 0 ? "Googlebot" : nil))
    }
